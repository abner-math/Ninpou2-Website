      ...
      <TableContainer component={Paper}>
        <Table size="small">
          ...
          {["KONOHAGAKURE", "OTOGAKURE", "AKATSUKI"].map((team) => (
            <TableBody key={team}>
              <TableRow>
                <StyledTableCell className={team} align="center" colSpan={10}>
                  {team}
                </StyledTableCell>
              </TableRow>
              {game.players
                  ...
                  >
                    ...
                    <TableCell>
                      <Avatar
                        variant="square"
                        alt={
                          player.hero.name.charAt(0).toUpperCase() +
                          player.hero.name.slice(1)
                        }
                        src={
                          "/images/heroes/npc_dota_hero_" +
                          player.hero.name +
                          ".png"
                        }
                      />
                    ...
                    <TableCell>
                      <Box sx={{ display: "flex" }}>
                        {player.items
                          .filter((item) => item.name !== "item_tpscroll")
                          .map((item) => (
                            <Avatar
                              key={item.name}
                              variant="square"
