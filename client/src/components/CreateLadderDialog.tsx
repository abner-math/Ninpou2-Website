...
export function CreateLadderDialog({ open, onOpenChanged, onLadderCreated }: CreateLadderProps) {
    ...
    const handleNameChange = (name: string) => {
        if (name.length < 3 || !name.match(/^[a-zA-Z0-9]+$/)) {
            setErrorMessage("Name must be at least three characters long and contain only letters and digits.");
        } else {
            setErrorMessage("");
        }
    };
    const handleCreate = async (name: string, passphrase: string): Promise<boolean> => {
